...
# Gradient descent performed on the ith value of the data set only
def gd_one_pass_one_value(w,data,i):
    return [ w[0] - L_RATE*dw0(w, [data[i]] ), w[1] - L_RATE*dw1(w, [data[i]] ), w[2] - L_RATE*dw2(w, [data[i]] ) ]
...
if __name__ == "__main__":
    ...
    for i in range(len(raw_data)):
        ...
        raw_data[i] = numpy.array( raw_data[i] )
    ...
    weights = numpy.zeros(3) # w0 = price constant, w1 = sq footage, w2 = beds
    ...
    while passes < PASS_LIMIT:
        ...
        """
        for i in range(len(raw_data)):
            weights = gd_one_pass_one_value(weights, raw_data, i)
            #print(weights)
        passes += 1
        print()
